...
void query(unsigned short qid, unsigned short artist, unsigned short areltd[], unsigned short bdstart, unsigned short bdend) {
	...
	memset(person_score, 0, person_length/sizeof(Person)); 
	...
	for (person_offset = 0; person_offset < person_length/sizeof(Person); person_offset++) {
		...
		person_score[person_offset] = score;
	}
	// now we iterate over all persons that could be friends
	for (friend_offset = 0; friend_offset < person_length/sizeof(Person); friend_offset++) {
		knows = &person_map[friend_offset];

		if (friend_offset > 0 && friend_offset % REPORTING_N == 0) {
			printf("%.2f%% pass(person.bin+knows.bin)\n", 100 * (friend_offset * 1.0/(person_length/sizeof(Person))));
		}

		// friend must already like the artist
		if (person_score[friend_offset] != -1) continue;

		for (knows_offset = knows->knows_first; 
			knows_offset < knows->knows_first + knows->knows_n; 
			knows_offset++) {
			person_offset = knows_map[knows_offset];
			person = &person_map[person_offset];

			// check the score of the person is sufficient
			if (person_score[person_offset] < 1 || person->location != knows->location) continue;
			...
			results[result_length].knows_id = friend_offset;
	...
	for (result_idx = 0; result_idx < result_length; result_idx++) {
		...
		for (knows_offset2 = person->knows_first; knows_offset2 < person->knows_first+person->knows_n; knows_offset2++) {
			if (knows_offset == knows_map[knows_offset2]) {
...
int main(int argc, char *argv[]) {
	...
	person_score = (char*) malloc(person_length*sizeof(char)/sizeof(Person)); 
